...
export function useTaskData() {
  ...
  // If status is set to "completed", the task is immediately archived.
      ...
      [queueId]: prev[queueId].map(t => {
        ...
        if (updates.status && updates.status !== t.status) {
          ...
          if (updates.status === "completed") {
            ...
            updated.completed_by  = user?.name ?? "Unknown";
  ...
  // Also updates queue.displayCols to include any new column names from the CSV
  // so the Flat File view, AddTaskModal form, and detail panel all stay current.
  const importToQueue = useCallback((queueId, csvRows) => {
    // Collect every field name present across all incoming rows
    const csvCols = [...new Set(csvRows.flatMap(r => Object.keys(r)))];
    ...
    // Merge new column names into the queue's displayCols so every part of the
    // UI knows about columns that arrived in this upload.
    setQueues(prev => prev.map(q => {
      if (q.id !== queueId) return q;
      const existing = q.displayCols ?? [];
      const existingSet = new Set(existing);
      const added = csvCols.filter(c => !existingSet.has(c) && !c.startsWith("_"));
      if (added.length === 0) return q; // nothing new
      return { ...q, displayCols: [...existing, ...added] };
    }));
