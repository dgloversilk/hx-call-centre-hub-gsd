...
export function useTaskData() {
  ...
  // Move a queue up or down in priority order (managers only).
  const reorderQueue = useCallback((queueId, direction) => {
    setQueues(prev => {
  ...
  // Move a queue from one position to another (for drag-and-drop).
  const moveQueue = useCallback((fromIdx, toIdx) => {
    setQueues(prev => {
